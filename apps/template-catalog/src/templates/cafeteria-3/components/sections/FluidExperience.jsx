...
const FluidExperience = () => {
  ...
  const services = [
    ...
    {
      icon: Palette,
      title: 'Galería Rotativa',
      subtitle: 'Arte Local',
      description: 'Exposiciones mensuales de artistas locales transforman constantemente nuestro espacio.',
      color: '#FF8B94',
      features: ['Artistas emergentes', 'Obras en venta', 'Eventos de apertura']
    },
    {
      icon: Users,
      title: 'Eventos Comunitarios',
      subtitle: 'Conexión Social',
      description: 'Talleres, catas y eventos que fomentan la comunidad cafetera local.',
      color: '#A8E6CF',
      features: ['Catas semanales', 'Talleres de latte art', 'Noches de juegos']
    }
  ]
    ...
    <section id="experience" className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-800 to-pink-800 relative overflow-hidden">
      ...
      <div className="relative z-10 min-h-screen flex">
        ...
        <div className="w-1/2 p-16 flex flex-col justify-center">
          ...
          <motion.p
            className="text-xl text-gray-300 mb-12 leading-relaxed max-w-md"
          ...
          >
            ...
            <div className="flex flex-wrap gap-2">
              {services[activeService].features.map((feature, index) => (
                <motion.span
                  key={index}
                  className="px-3 py-1 bg-white/20 rounded-full text-sm text-white border border-white/30"
            ...
            {services.map((_, index) => (
              <motion.button
                ...
                className={`w-3 h-3 rounded-full transition-all duration-300 ${
                  index === activeService ? 'bg-white scale-125' : 'bg-white/30'
                }`}
        ...
        {/* Right side - Interactive service grid */}
        <div className="w-1/2 p-16 flex items-center">
          <div className="grid grid-cols-2 gap-6 w-full">
            {services.map((service, index) => (
              <motion.div
                key={index}
                className={`relative bg-white/5 backdrop-blur-lg rounded-2xl p-6 border cursor-pointer transition-all duration-500 ${
                  index === activeService 
